...
import { generateRawUserSummary, type RawUserSummaryResponse } from './generate-raw-user-summary.js';
...
describe('generateRawUserSummary', () => {
  ...
  const rawUSDCSummaryModified: UserReserveSummaryResponse = generateUserReserveSummary({
    userReserve: {
      ...usdcUserMock.userReserve,
      scaledATokenBalance: (Number(usdcUserMock.userReserve.scaledATokenBalance) * 2).toString(),
      reserve: usdcUserMock.reserve,
    },
    marketReferencePriceInUsdNormalized,
    marketReferenceCurrencyDecimals: 18,
  ...
  const rawETHSummaryModified: UserReserveSummaryResponse = generateUserReserveSummary({
    userReserve: {
      ...ethUserMock.userReserve,
      scaledVariableDebt: (Number(ethUserMock.userReserve.scaledVariableDebt) * 2).toString(),
      reserve: {
        ...ethUserMock.reserve,
  ...
  it('should generate the correct user summary ', () => {
    ...
    expect(normalize(rawSummary.totalLiquidityUSD, marketReferenceCurrencyDecimals)).toEqual('40000');
    expect(normalize(rawSummary.totalCollateralUSD, marketReferenceCurrencyDecimals)).toEqual('40000');
    expect(normalize(rawSummary.totalBorrowsUSD, marketReferenceCurrencyDecimals)).toEqual('20000');
    expect(normalize(rawSummary.totalLiquidityMarketReferenceCurrency, marketReferenceCurrencyDecimals)).toEqual(
      '4000',
    );
    expect(normalize(rawSummary.totalCollateralMarketReferenceCurrency, marketReferenceCurrencyDecimals)).toEqual(
      '4000',
    );
    expect(normalize(rawSummary.totalBorrowsMarketReferenceCurrency, marketReferenceCurrencyDecimals)).toEqual('2000');
    expect(normalize(rawSummary.availableBorrowsMarketReferenceCurrency, marketReferenceCurrencyDecimals)).toEqual('0');
    expect(normalize(rawSummary.availableBorrowsUSD, marketReferenceCurrencyDecimals)).toEqual('0');
  ...
  it('should not allow negative available borrows if debtCeiling is exceeded ', () => {
    const rawETHSummaryDebtCeilingExceeded: UserReserveSummaryResponse = generateUserReserveSummary({
      userReserve: {
        ...ethUserMock.userReserve,
        reserve: {
          ...ethUserMock.reserve,
    ...
    const rawSummaryDebtCeilingExceeded: RawUserSummaryResponse = generateRawUserSummary({
    ...
    expect(
      normalize(rawSummaryDebtCeilingExceeded.availableBorrowsMarketReferenceCurrency, marketReferenceCurrencyDecimals),
    ).toEqual('0');
  });

  it('should generate the correct user summary for user in stablecoin eMode', () => {
    expect(normalize(rawSummaryEMode.availableBorrowsMarketReferenceCurrency, marketReferenceCurrencyDecimals)).toEqual(
      '400',
    );
    expect(normalize(rawSummaryEMode.availableBorrowsUSD, marketReferenceCurrencyDecimals)).toEqual('4000');
    expect(rawSummaryEMode.currentLoanToValue.toFixed()).toEqual('6000');
    expect(rawSummaryEMode.currentLiquidationThreshold.toFixed()).toEqual('7000');
