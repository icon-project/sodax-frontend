...
describe('calculateUserReserveTotals', () => {
  ...
  const usdcReserveMock = new UserReserveMock({ decimals: 6 }).supply(200).variableBorrow(100);
  ...
  const ethReserveMock = new UserReserveMock({ decimals: 18 }).supply(200).variableBorrow(100);
  ...
  it('should compute totals from user reserve array', () => {
    ...
    expect(
      userReserveTotals.totalLiquidityMarketReferenceCurrency.shiftedBy(-marketReferenceCurrencyDecimals).toString(),
    ).toEqual('4000');
    expect(
      userReserveTotals.totalBorrowsMarketReferenceCurrency.shiftedBy(-marketReferenceCurrencyDecimals).toString(),
    ).toEqual('2000');
    expect(
      userReserveTotals.totalCollateralMarketReferenceCurrency.shiftedBy(-marketReferenceCurrencyDecimals).toString(),
    ).toEqual('4000');
    expect(userReserveTotals.currentLtv.toFixed()).toEqual('5000');
    expect(userReserveTotals.currentLiquidationThreshold.toFixed()).toEqual('6000');
    ...
    expect(userReserveEModeTotals.currentLiquidationThreshold.toFixed()).toEqual('7000');
  });
