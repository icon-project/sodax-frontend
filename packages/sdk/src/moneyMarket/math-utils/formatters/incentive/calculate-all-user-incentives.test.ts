...
import { calculateAllUserIncentives, type CalculateAllUserIncentivesRequest } from './calculate-all-user-incentives.js';
...
describe('calculateAllUserIncentives', () => {
  ...
  it('should calculate the correct incentives for all reserves of mock user', () => {
    ...
    const claimable = result['0x0000000000000000000000000000000000000000']?.claimableRewards;
    // accrued + unclaimed rewards
    expect(normalize(claimable ?? 0, 18)).toBe('200000000000.000000000000000001');
  });
  ...
  it('should return empty if reserve incentives missing', () => {
    const totalUserIncentivesWithoutReserveIncentives: CalculateAllUserIncentivesRequest = {
      reserveIncentives: [{ ...reserveIncentiveMock.reserveIncentive, underlyingAsset: '0x0' }],
      userIncentives: [userIncentiveMock.userIncentive],
      userReserves: [
        {
          ...userMock.userReserve,
    ...
    const result = calculateAllUserIncentives(totalUserIncentivesWithoutReserveIncentives);
  ...
  it('should return userUnclaimedRewards even if userReserves is empty', () => {
    ...
    const result = calculateAllUserIncentives(totalUserIncentivesWithoutUserReserves);
    const claimable = result['0x0000000000000000000000000000000000000000']?.claimableRewards;
    // only unclaimed rewards
    expect(normalize(claimable ?? 0, 18)).toBe('0.000000000000000001');
  });
});
