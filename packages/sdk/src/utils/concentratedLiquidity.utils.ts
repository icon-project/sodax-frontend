...
export { getPoolId, encodePoolKey, decodeCLPoolParameters, TickMath };
...
export async function calculateLiquidityParams(
  ...
  // Handle both hex string and CLPoolParameter types
  const tickSpacing = typeof poolKey.parameters === 'string' ? getTickSpacing(poolKey.parameters) : 0; // Default fallback
...
export function buildPoolKey({
  ...
  // Create CLPoolParameter object
  const clPoolParameter: CLPoolParameter = {
    tickSpacing,
    hooksRegistration: {}, // Empty hooks registration for now
  };
...
export function getTickSpacing(parameters: `0x${string}`): number {
  const clParams = decodeCLPoolParameters(parameters);
  return clParams.tickSpacing;
}
