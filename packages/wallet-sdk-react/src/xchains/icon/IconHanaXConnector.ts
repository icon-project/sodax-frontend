...
export class IconHanaXConnector extends XConnector {
  ...
  async connect(): Promise<XAccount | undefined> {
    ...
    if (!hanaWallet || !hanaWallet.available) {
      // Throw instead of silently navigating — callers that bypass
      // `useWalletModal.selectWallet`'s pre-check (e.g. direct
      // `useXConnect` or `useConnectionFlow` usage) otherwise open a
      // tab with no surfaced error. Consumers render the install CTA
      // via `connector.installUrl` on the caught error.
      throw new Error('Hana Wallet is not installed. Install the extension and reload the page.');
    }
