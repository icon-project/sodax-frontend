...
import { EvmWalletProvider } from './wallet-providers.js';
...
async function supply(token: Address, amount: bigint) {
  const wallet = spokeProvider.walletProvider.getWalletAddress();
...
async function borrow(token: Address, amount: bigint) {
  ...
  const txHash = await SonicSpokeService.callWallet(wallet, data, spokeProvider, hubProvider);
...
async function withdraw(token: Address, amount: bigint) {
  ...
  const txHash = await SonicSpokeService.callWallet(wallet, withdrawData, spokeProvider, hubProvider);
