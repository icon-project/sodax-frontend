// apps/demo/src/components/dex/PositionViewer.tsx
...
export function PositionViewer() {
  ...
  const fetchPools = useCallback(async () => {
    ...
    try {
      // Create a new hub provider for this operation
      ...
      const poolDataPromises = availablePools.map(async poolKey => {
        try {
          const poolData = await clService.getPoolData(poolKey, hubProvider.publicClient);
          return poolData;
        } catch (err) {
          ...
          {poolDataList.length > 0 && (
            <div className="space-y-4">
              <div className="flex items-center justify-between p-4 bg-muted/30 rounded-lg">
                ...
                <div className="text-sm font-medium">Total Pools: {poolDataList.length}</div>
              </div>
