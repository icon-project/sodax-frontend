...
export function useReserveMetrics({
  ...
  try {
    ...
    if (reserve) {
      ...
      const raw_total_supply = formatUnits(total, token.decimals);
      const raw_total_borrow = formatUnits(borrowed, token.decimals);

      totalSupply = formatCompactNumber(Number.parseFloat(raw_total_supply));
      totalBorrow = formatCompactNumber(Number.parseFloat(raw_total_borrow));
