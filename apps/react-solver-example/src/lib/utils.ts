...
export function scaleTokenAmount(amount: number | string, decimals: number): bigint {
  return BigInt(new BigNumber(amount.toString()).multipliedBy(new BigNumber(10).pow(decimals)).toFixed(0, BigNumber.ROUND_DOWN));
}

export function normaliseTokenAmount(amount: number | string | bigint, decimals: number): string {
  return (new BigNumber(amount.toString()).dividedBy(new BigNumber(10).pow(decimals))).toFixed(decimals, BigNumber.ROUND_DOWN);
}
