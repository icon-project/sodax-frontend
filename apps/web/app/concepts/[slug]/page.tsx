...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const data = await getNotionPageBySlug('concepts', slug);
  if (!data) return {};
  ...
  const url = `https://sodax.com/concepts/${slug}`;

  return {
    title: `${title} - SODAX Concept | Cross-Chain DeFi Guide`,
    description: `${description} Understand ${title} in cross-chain DeFi with SODAX.`,
    keywords: ['SODAX', title, 'DeFi concept', 'cross-chain', 'blockchain', ...tags],
    authors: [{ name: 'SODAX Team' }],
    openGraph: {
      title: `${title} - SODAX Concept`,
      description,
      url,
      siteName: 'SODAX',
      type: 'article',
...
export default async function ConceptPage({ params }: Props) {
  const { slug } = await params;
  const [data, glossaryTerms] = await Promise.all([getNotionPageBySlug('concepts', slug), getGlossaryTerms()]);
  ...
  const structuredData = {
    ...
    '@graph': [
      {
        '@type': 'Article',
        '@id': `https://sodax.com/concepts/${slug}`,
        headline: title,
        description: summary,
      ...
      {
        '@type': 'BreadcrumbList',
        '@id': `https://sodax.com/concepts/${slug}#breadcrumb`,
        itemListElement: [
          {
            '@type': 'ListItem',
            position: 1,
                  ...
                  <div
                    ...
                    dangerouslySetInnerHTML={{
                      __html: injectGlossaryLinks(
                        content,
                        glossaryTerms.filter(t => t.url !== `/concepts/${slug}`),
                        1,
                      ),
                    }}
                  />
