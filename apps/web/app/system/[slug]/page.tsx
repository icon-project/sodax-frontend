...
import { getNotionPageBySlug, getAllSlugs } from '@/lib/notion';
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const url = `https://sodax.com/system/${slug}`;

  return {
    title: `${title} - SODAX System Component | Cross-Chain DeFi Documentation`,
    description: `${description} Learn about ${title} in the SODAX cross-chain DeFi system.`,
    keywords: ['SODAX', title, 'system component', 'cross-chain', 'DeFi', ...tags],
    authors: [{ name: 'SODAX Team' }],
    openGraph: {
      title: `${title} - SODAX System Component`,
      description,
      url,
      siteName: 'SODAX',
      type: 'article',
      ...
      images: [
        {
          url: '/og-system-component.png',
          width: 1200,
          height: 630,
          alt: `${title} - SODAX System Component`,
        },
      ],
    },
    twitter: {
      ...
      title: `${title} - SODAX System Component`,
      description,
      images: ['/og-system-component.png'],
      creator: '@gosodax',
    },
    alternates: {
      canonical: url,
...
export default async function SystemPage({ params }: Props) {
  const { slug } = await params;
  const [data, glossaryTerms] = await Promise.all([getNotionPageBySlug('system', slug), getGlossaryTerms()]);
  ...
  const structuredData = {
    ...
    '@graph': [
      {
        '@type': 'TechArticle',
        '@id': `https://sodax.com/system/${slug}`,
        headline: title,
        description: summary,
        ...
        about: {
          ...
          name: 'SODAX',
          applicationCategory: 'DeFi Platform',
        },
      },
      {
        '@type': 'BreadcrumbList',
        '@id': `https://sodax.com/system/${slug}#breadcrumb`,
        itemListElement: [
          {
            '@type': 'ListItem',
            position: 1,
                  ...
                  <div
                    ...
                    dangerouslySetInnerHTML={{
                      __html: injectGlossaryLinks(
                        content,
                        glossaryTerms.filter(t => t.url !== `/system/${slug}`),
                        1,
                      ),
                    }}
                  />
