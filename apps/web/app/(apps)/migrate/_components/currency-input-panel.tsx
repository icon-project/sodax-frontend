...
const CurrencyInputPanel: React.FC<CurrencyInputPanelProps> = ({
  ...
  return (
    <div className="relative w-full group">
      {/* Border layer with multiply blend mode */}
      <div
        className={`absolute inset-0 rounded-(--layout-container-radius) outline-almost-white pointer-events-none ${className} outline-2 outline-offset-[-2px] group-hover:outline-4 group-hover:outline-offset-[-4px] sm:outline-3 sm:outline-offset-[-3px] sm:group-hover:outline-5 sm:group-hover:outline-offset-[-5px] md:outline-4 md:outline-offset-[-4px] md:group-hover:outline-6 md:group-hover:outline-offset-[-6px]`}
        style={{ mixBlendMode: 'multiply' }}
      />
      {/* Content layer without blend mode */}
      <div
        className="w-full relative rounded-(--layout-container-radius) inline-flex justify-between items-center"
  ...
  translate-y-[2px] sm:translate-y-[1px] md:translate-y-[2px]"
            ...
            <div className="text-right justify-center text-espresso font-['InterRegular'] font-normal text-(length:--body-super-comfortable) flex-none">
              <span className="inline-flex items-baseline leading-none relative top-0.5">
                <span className="font-['InterRegular']">{isBnUSD ? 'bnUSD' : currency.symbol}</span>
                {tokenLabel && (
                  <span className="text-negative ml-1 font-['InterBold'] text-(size:--body-tiny) relative -top-0.5 leading-none">
                    {tokenLabel}
                  </span>
                )}
              </span>
